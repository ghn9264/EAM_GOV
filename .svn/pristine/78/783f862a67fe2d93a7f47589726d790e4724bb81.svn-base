...
{
    public class AssetsMainRepository : Repository<AssetsMain, int>, IAssetsMainRepository
    {
        public AssetsMainRepository(EamDatabase database)
            : base(database)
        {
        ...
        public List<AssetsMain> LogicDeleteEx(int import_id)
        {
            //
            ...
            // 删除主表资产
            // 
            Db.Execute("UPDATE ASSETS_MAIN SET IS_DELETE=1 WHERE IMPORT_ID=@0", import_id);
            return Query(sql);
        }

        public int ChangeInfo(int entityId, string item, string content)
        {
            return Db.Execute("UPDATE ASSETS_MAIN SET " + item + "=@0 WHERE ID=@1", content, entityId);
        }
