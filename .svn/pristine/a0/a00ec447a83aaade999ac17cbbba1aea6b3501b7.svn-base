...
{
    public class ExportHouse : ExportAssetsBase
    {
        public ExportHouse(IAssetsService assetsService) : base(assetsService)
        {
        ...
        {
            get { return AssetsTypes.House; }
        }

        public override string SaveFileName
        {
            get { return @"ZC02_房屋_导入模板.xls"; }
        }
        ...
        protected override void FillBody()
        {
            ...
            for(int index=0;index<AssetsData.Count;index++)
            {
                ...
                var attrData = AssetsService.GetHouse(main.AssetsNum);
                if (null != attrData)
                {
                    TempRow.CreateCell(9).SetCellValue(attrData.PropertyForm);
                    TempRow.CreateCell(10).SetCellValue(attrData.OwnershipCertificate);
                    TempRow.CreateCell(11).SetCellValue(attrData.OwnershipCertifiateNum);
                    TempRow.CreateCell(12).SetCellValue(attrData.OwnershipTime);
                    TempRow.CreateCell(13).SetCellValue(attrData.IssuingDate.ToShortDateString());
                    TempRow.CreateCell(14).SetCellValue(attrData.HouseOwner);
                    TempRow.CreateCell(15).SetCellValue(attrData.DesignUse);
                    TempRow.CreateCell(16).SetCellValue(attrData.BuildingStruccture);
                    TempRow.CreateCell(17).SetCellValue(attrData.Location);
                    TempRow.CreateCell(18).SetCellValue(attrData.Area.ToString());
                }
                ...
                TempRow.CreateCell(23).SetCellValue(main.Depreciation.ToString());
                TempRow.CreateCell(24).SetCellValue(main.AccumulateDepreciation.ToString());
                TempRow.CreateCell(25).SetCellValue(main.NetWorth.ToString());
                ...
                TempRow.CreateCell(28).SetCellValue(main.Memo);
                NextRowIndex++;
                Thread.Sleep(10);
            }
        }
